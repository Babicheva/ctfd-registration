...
from src.utils import verify_hcaptcha, valid_symbols, verify_password, valid_telegram_tag, valid_age, send_email_async
from src.config import get_secret_key, get_event_name, get_event_description, get_logo_path, get_event_link
...
def success():
    context = {
        'subject': 'Успех',
        'title': 'Успех',
        'logo_path': url_for('static', filename=LOGO_PATH),
    }
...
def register_user():
    ...
    if form.validate_on_submit():
        ...
        if new_user == "duplicate_email":
            flash("Пользователь с таким email уже существует.")
            return render_template('register.html', form=form, context=context)
        elif new_user == "database_error":
            flash("Ошибка сервера при регистрации. Попробуйте позже.")
...
def join_team():
    ...
    if form.validate_on_submit():
        ...
        if success_join_team is True:
            ...
            threading.Thread(target=send_email_async, args=(current_user.get_email(), context)).start()
